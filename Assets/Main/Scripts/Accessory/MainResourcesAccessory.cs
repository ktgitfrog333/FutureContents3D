...
namespace Main.Accessory
{
    /// <summary>
    /// リソースアクセス
    /// タイトル用
    /// </summary>
    public class MainResourcesAccessory
    {
        /// <summary>
        /// 初期処理
        /// </summary>
        public void Initialize()
        {
            ...
            // ステージ設定ファイルが存在しない場合は作成
            if (!File.Exists($"{GetHomePath()}{ConstResorcesNames.MAIN_SCENE_STAGES_CONFIG}.csv"))
            {
                using (File.Create($"{GetHomePath()}{ConstResorcesNames.MAIN_SCENE_STAGES_CONFIG}.csv")) { }
            }
        }
        ...
        public Dictionary<EnumMainSceneStagesConfig, int>[] GetMainSceneStagesConfig(List<string[]> datas)
        {
            try
            {
                var configMapList = new List<Dictionary<EnumMainSceneStagesConfig, int>>();
                // 格納する配列0番目は空をセット（シーンIDとのズレを無くすため）
                configMapList.Add(new Dictionary<EnumMainSceneStagesConfig, int>());
                // 一行目はカラム名なのでスキップ
                for (var i = 1; i < datas.Count; i++)
                {
                    var child = datas[i];
                    var configMap = new Dictionary<EnumMainSceneStagesConfig, int>();
                    for (var j = 0; j < child.Length; j++)
                    {
                        configMap[(EnumMainSceneStagesConfig)j] = int.Parse(child[j]);
                    }
